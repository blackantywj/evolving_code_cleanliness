...
def analysis(repository):
    ...
    for commit in commits:
        ...
        if commit['commit_sha'] != "559f12154a40f3bf6944c84e808f488e9f36242d":
            continue
...
def summary(repository, language: str = "all", start_time: str = None, end_time: str = None):
    ...
    for commit in commits:
        origin = commit_collection.find_one({"repository": repository, "commit_sha": "559f12154a40f3bf6944c84e808f488e9f36242d"})
        if commit["_id"] != origin["_id"]:
            continue
...
def analysis_with_commit(repository, commit_cha):
    origin = commit_collection.find_one({"repository": repository, "commit_sha": commit_cha})
    print(origin)
...
if __name__ == "__main__":
    ...
    threshold(repository, chars=80)
    threshold(repository, circle=10)
    threshold(repository, method_rows=30)
    threshold(repository, file_rows=2000)
    # search(repository, start_time, end_time)
   ...
   #  summary(repository, language="java")
    ...
    # analysis_with_commit(repository, commit_cha = "96703056692813651cf5aa47286443d603f507bd")
