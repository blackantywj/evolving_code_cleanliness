...
tokenizer = AutoTokenizer.from_pretrained("/home/cumt/workspace/code_analysis/github_commits/codebert")
model = AutoModelForMaskedLM.from_pretrained("/home/cumt/workspace/code_analysis/github_commits/codebert")
...
def analysis_variable(repository, start_time, end_time):
    ...
    for commit in commits:
        ...
        for data in datas:
            ...
            if fileType == 'java':
                ...
                # 调用 Java 方法提取变量名
                try:
                    new_variable_names = java_parser_example.extractVariableNames(newFileContent)
                    old_variable_names = java_parser_example.extractVariableNames(oldFileContent)
                    if new_variable_names == [] or old_variable_names == []:
                        continue
                    else:
                        new_variable_names_list = new_variable_names.split(" ")
                        old_variable_names_list = old_variable_names.split(" ")
                        new_entropy = compute_entropy(new_variable_names_list)
                        old_entropy = compute_entropy(old_variable_names_list)
                        old_data.append(old_entropy)
                        new_data.append(new_entropy)
                        # if new_entropy > 7 and len(new_variable_names_list) < 170:
                        #     print(new_variable_names_list)
                        # if old_entropy > 7 and len(old_variable_names_list) < 170:
                        #     print(old_variable_names_list)
    ...
    # 创建直方图
    plt.hist(old_data, bins=30, edgecolor='black', alpha=0.7)
...
def compute_entropy(new_variable_names_list):
    variable_names_tokens = tokenizer(new_variable_names_list, return_tensors="pt", truncation=True, padding=True)
    variable_names_tokens = variable_names_tokens['input_ids']
    variable_names_embeddings = model.base_model.embeddings.word_embeddings(variable_names_tokens).detach().numpy()
    variable_names_embeddings = variable_names_embeddings.reshape(variable_names_tokens.shape[0], -1)
    # 2. 对所有Embeddings向量进行归一化
    normalized_embeddings = normalize(variable_names_embeddings)

    # 3. 计算每个Embedding的频率分布
    embedding_freqs = Counter(map(tuple, normalized_embeddings))

    # 4. 计算每个向量的概率分布
    total_embeddings = len(normalized_embeddings)
    probabilities = np.array([count / total_embeddings for count in embedding_freqs.values()])
...
if __name__ == "__main__":
    ...
    # # 添加命令行参数
    # # 仓库名，可以选择多个Apache项目
    # parser.add_argument('-r', '--repository', type=str, default='apache/rocketmq',
    ...
    # summary(repository, language="java")
