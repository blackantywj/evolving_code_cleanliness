...
commit_collection = get_collection('github_commit_2026_c_redis_test')
print("total docs =", commit_collection.estimated_document_count())
print("any doc =", commit_collection.find_one())
...
TOKENS = []  # api token 可以添加多个，提高访问速度 3866628937@qq.com
...
#     return response
def get_response(url, params=None, headers=None, max_retries=5):
    """封装 HTTP 请求，处理限流、重试和 Token 轮换"""
    retries = 0
    while retries < max_retries:
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"⚠️  Token 可能失效，切换 Token 重试...")
                retries += 1
                time.sleep(1)
            else:
                print(f"❌ 请求失败 (状态码: {response.status_code}): {response.text[:200]}")
                retries += 1
                time.sleep(2)

        except requests.exceptions.RequestException as e:
            print(f"🌐 网络异常: {str(e)}，重试中...")
            retries += 1
            time.sleep(2)

    raise Exception(f"❌ 超过最大重试次数，请求失败: {url}")
...
def update_token_remaining(token):
    ...
    try:
        ...
        rate_limit_data = response.json()
        # 修复3：容错KeyError，逐层判断
        remaining = rate_limit_data.get('resources', {}).get('core', {}).get('remaining', 0)
        ...
        # 网络异常：超时/连接失败/4xx/5xx
        print(f"Token {token[:8]}... 网络异常: {str(e)[:50]}")
        with token_lock:
            token_remaining[token] = 0
    except (json.JSONDecodeError, KeyError) as e:
        # 解析异常：JSON格式错误/字段缺失
        print(f"Token {token[:8]}... 解析异常: {e}")
        with token_lock:
            token_remaining[token] = 0
    except Exception as e:
        # 兜底捕获所有异常，避免线程退出
        print(f"Token {token[:8]}... 未知异常: {e}")
        with token_lock:
            token_remaining[token] = 0
...
def check_rate_limit():
    ...
    while not stop_event.is_set():
        try:
            # 遍历token时加锁（短时间锁，不影响性能）
            with token_lock:
                tokens = list(TOKENS)  # 复制列表，避免遍历中修改
            for token in tokens:
                update_token_remaining(token)
            
            # 修复：sleep前检查退出信号，避免sleep期间无法退出
            for _ in range(10):
...
def get_first_commit(repository):
    """
    【恢复原函数】获取仓库默认分支的最新一条 commit 的 sha
    :param repository: 仓库名称
    :return: 最新的 commit 的 sha
    """
...
#         # print("exists True =", commit_collection.count_documents({"repository": repository, "download": {"$exists": True}}))
#         # print("exists False =", commit_collection.count_documents({"repository": repository, "download": {"$exists": False}}))
#         # print("download is None =", commit_collection.count_documents({"repository": repository, "download": None}))
#         # print("sample with download field =", commit_collection.find_one({"repository": repository, "download": {"$exists": True}}, {"download": 1}))
#         # for commit in commits:
#         #     download_concurrent(commit)
...
def download(repository):
    try:
        # 预加载所有待处理的commit到列表（一次性拉取）
        commits = list(commit_collection.find({"repository": repository, "download": {"$exists": False}}))
        if not commits:
            return
        ...
        with ThreadPoolExecutor(max_workers=25) as executor:
            futures = []
            for commit in commits:
                futures.append(executor.submit(download_concurrent, commit))
...
def process_files_concurrently(files, parents_sha):
    ...
    with ThreadPoolExecutor(max_workers=3) as executor:
        futures = [executor.submit(process_file, file, parents_sha) for file in files]
        for future in as_completed(futures):
            result = future.result()
            if result is not None:
                datas.append(result)
...
#     results = response.json()
#     # e = time() 
#     # print(f"get_commits|{repository}|{sha}|{start_time}|{end_time}|{e-s:.2f}s")
#     # pprint(results)
#     if not results:
#         return
...
#         # e_find = time()
#         # print(f"find_one|{repository}|{commit_sha}|{e_find-s_find:.2f}s")
#         if not origin:
#             # s_insert = time()
#             commit_collection.insert_one(data)
#             # e_insert = time()
#             # print(f"insert_one|{repository}|{commit_sha}|{e_insert-s_insert:.2f}s")
#     last_sha = results[-1]["sha"]
#     last_parents_sha = results[-1]["parents"]
#     # print(last_parents_sha)
#     if last_parents_sha:
#         # if type(last_parents_sha) == list and len(last_parents_sha) != 0:
...
def batch_insert(batch_data):
    if not batch_data:
        return 0
    try:
        result = commit_collection.insert_many(batch_data, ordered=False)
        return len(result.inserted_ids)
    except errors.BulkWriteError as e:
        return e.details.get("nInserted", 0)


def find_closest_earlier_commit(repository, current_sha, current_time_str, processed_shas):
    url = f"https://api.github.com/repos/{repository}/commits"
    params = {"per_page": 20, "until": current_time_str}
    response = get_response(url, params=params)
    commits = response.json()
    for commit in commits:
        candidate_sha = commit["sha"]
        if candidate_sha != current_sha and candidate_sha not in processed_shas:
            return candidate_sha
    return None


def process_single_commit(repository, commit_sha):
    """
    【核心优化】只处理单个提交：调用 /commits/:sha 接口
    返回：(commit_data, parent_shas)
    """
    url = f"https://api.github.com/repos/{repository}/commits/{commit_sha}"
    response = get_response(url)
    ...
    # 解析数据
    author_date_str = commit["commit"]["author"]["date"]
    author_dt = datetime.strptime(author_date_str, "%Y-%m-%dT%H:%M:%SZ")
    commit_date = author_dt.date()
    
    github_author_name = commit["author"]["login"] if commit["author"] else None
    github_committer_name = commit["committer"]["login"] if commit["committer"] else None
    parent_shas = [p["sha"] for p in commit["parents"]]
    
    commit_data = {
        "repository": repository,
        "commit_sha": commit_sha,
        "release_time": commit_date.isoformat(),
        "author": {
    ...
    return commit_data, parent_shas, author_date_str


def search(repository, start_time=None, end_time=None):
    if not re.match(r"^[^/]+/[^/]+$", repository):
        raise ValueError("repository 格式错误")
    ...
    print(f"🚀 开始最终优化爬取: {repository}")
    print(f"   策略：仅初始起点翻页，其余仅查单条")
    ...
    # ==========================================
    # 【第一阶段】仅对初始起点（默认分支最新提交）处理全部分页
    # 这是唯一需要翻页的地方
    # ==========================================
    print(f"\n📄 【阶段 1/2】处理默认分支全量历史（唯一翻页阶段）...")
    initial_url = f"https://api.github.com/repos/{repository}/commits"
    initial_params = {"per_page": PER_PAGE}
    
    while initial_url:
        response = get_response(initial_url, params=initial_params)
        total_api_calls += 1
        commits = response.json()
        ...
        for commit in commits:
            ...
            if commit_sha in processed_shas:
                continue
            ...
            batch_data.append(commit_data)

            # 【关键】将父节点加入队列（留到第二阶段处理，不翻页）
            for parent_sha in parent_shas:
                if parent_sha not in processed_shas:
            ...
            if len(batch_data) >= BATCH_SIZE:
                inserted = batch_insert(batch_data)
                total_inserted += inserted
                print(f"   💾 批量插入: {inserted}/{len(batch_data)} | 总计: {total_inserted}")
                batch_data = []

            # 处理中间根（时间跳转）
            if len(parent_shas) == 0:
                next_sha = find_closest_earlier_commit(
                    repository, commit_sha, author_date_str, processed_shas
                )
                if next_sha and next_sha not in processed_shas:
                    queue.append(next_sha)
                    print(f"   🔄 发现中间根，加入队列: {next_sha[:7]}")

        # 下一页
        initial_url = response.links.get("next", {}).get("url")
        initial_params = None

    # ==========================================
    # 【第二阶段】处理队列里的所有 SHA，仅查单条，绝不翻页
    # ==========================================
    print(f"\n🔍 【阶段 2/2】处理队列中的 {len(queue)} 个 SHA（仅查单条）...")
    ...
    while queue:
        ...
        # 【核心】只查单条，不翻页
        try:
            commit_data, parent_shas, author_date_str = process_single_commit(repository, commit_sha)
            total_api_calls += 1
        except Exception as e:
            print(f"   ❌ 获取单条提交失败 {commit_sha[:7]}: {e}")
            continue
        
        # 时间过滤
        commit_date = commit_data["author"]["date"].date()
        if start_dt and commit_date < start_dt:
            processed_shas.add(commit_sha)
            continue
        if end_dt and commit_date > end_dt:
        ...
        if len(batch_data) >= BATCH_SIZE:
            inserted = batch_insert(batch_data)
            total_inserted += inserted
            print(f"   💾 批量插入: {inserted}/{len(batch_data)} | 总计: {total_inserted} | 队列剩余: {len(queue)}")
            batch_data = []
    ...
    if batch_data:
        inserted = batch_insert(batch_data)
        total_inserted += inserted
        print(f"💾 插入剩余数据: {inserted}/{len(batch_data)}")

    # 统计输出
    elapsed = time.time() - start_time_total
    print(f"\n🎉 爬取完成！")
    print(f"   总处理提交: {total_processed}")
    print(f"   新增入库: {total_inserted}")
    print(f"   总 API 调用: {total_api_calls}")
    print(f"   总耗时: {elapsed:.2f}s")
    print(f"   平均速度: {total_processed/elapsed:.2f} 提交/秒")
...
# 修复线程启动：改为非守护线程 + 主线程等待（可选）
def start_rate_monitor():
    rate_thread = threading.Thread(target=check_rate_limit)
    rate_thread.start()
    return rate_thread
...
if __name__ == "__main__":
    ...
    # 结束时间，可以选择时间范围
    parser.add_argument('-e', '--end_time', type=str, default='2026-01-01',
    ...
    #    rate_limit_thread.start()  # Start the rate limit checking thread
       rate_thread = start_rate_monitor()
