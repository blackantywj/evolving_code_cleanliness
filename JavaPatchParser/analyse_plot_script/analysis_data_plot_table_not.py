...
if __name__ == "__main__":
    ...
    for mnr in args.my_need_repo:
        # 输出解析结果
        file_path = 'circle/data_apache_with_true_committer/%s/detail.txt' % args.element  # 你可以将此文件路径替换为实际文件路径
        ...
        #         need_plot_commit_p[mnr.split('/')[0]][person] = commits

        # # 遍历每个提交人，绘制他们的提交次数随时间的变化
        # for person, commits in commit_counts_n.items():
        #     if person == author_with_commit_n_name[0][0]:
        #         need_plot_commit_n[mnr.split('/')[0]][person] = commits
    ...
    plt.savefig(f"committer_plot/{mnr.split('/')[0]}_{args.element}.png")
