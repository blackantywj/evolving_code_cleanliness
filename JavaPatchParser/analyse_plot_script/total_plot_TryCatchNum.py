...
#     # 绘制主轴上的 values1
#     ax1.plot(months, values1, label=f'{project[i]}/-', color=colors[i], alpha=0.3, linewidth=2, linestyle='--', marker='x')
...
#     values3 = list(data['values3'].values())
    ...
    ax2.plot(months, values2, label=f'{project[i]}/↑', color=colors[i], alpha=1.0, linewidth=2, marker='o')
    ax2.plot(months, values3, label=f'{project[i]}/↓', color=colors[i], alpha=1.0, linewidth=2, linestyle='--', marker='x')

# 添加标签和标题
ax2.set_xlabel('Year-Month', fontsize=30)
# ax1.set_ylabel('Counts (-)', fontsize=12, color='black')
ax2.set_ylabel('Counts', fontsize=30, color='black')
ax2.set_title(f'{argument}', fontsize=30)

# # 调整 X 轴刻度
# plt.xticks(rotation=60, fontsize=1)  # 标签旋转 45° 并右对齐
ax2.set_xticks(range(len(months)))  # 确保刻度与数据对齐
ax2.set_xticklabels(months, rotation=45, fontsize=16, ha='right')  # 旋转标签并右对齐
...
ax2.legend(lines2, labels2, loc='upper left', fontsize=16)
