...
if __name__ == "__main__":
    ...
    parser.add_argument("-e", "--element", type=str, default="TryCatchNum")
    parser.add_argument("-c", "--condition", type=str, default="Try-Catch个数变化")
    ...
    # 输出解析结果
    file_path = '/home/cumt/workspace/code_analysis/JavaPatchParser/circle/data_apache_4+/%s/detail.txt' % args.element  # 你可以将此文件路径替换为实际文件路径
    # 按月份统计
    ...
    with open(f'{args.element}_{args.my_need_repo[0].split("/")[0]}.json', 'w') as f:
        json.dump({'months': all_months, 'values1': values1, 'values2': values2, 'values3':values3}, f)
    # # # 打印结果
    # # 获取统一时间轴
    # all_months = sorted(set(monthly_stats.keys()).union(monthly_stats_p.keys()))
